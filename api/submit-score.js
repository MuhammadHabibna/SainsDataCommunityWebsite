...
export default async function handler(req, res) {
    ...
    // SANITIZATION: Strip HTML tags and limit length
    const sanitizedPlayerName = playerName.replace(/<[^>]*>?/gm, "").trim().slice(0, 20);
    ...
    for (const answer of answers) {
        ...
        if (question) {
            if (question.answer === answer.selectedAnswerIndex) {
                // Correct Answer
                currentStreak++;
    ...
    const { data, error } = await supabase
        .from('leaderboard')
        .insert([{ name: sanitizedPlayerName, score: calculatedScore }])
        .select();
